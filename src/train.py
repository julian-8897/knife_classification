...
import timm
from utils import *
from model import FineTunedResNetV1, FineTunedDenseNet, ConvNet, FineTunedEffNetV2
...
# model = timm.create_model('tf_efficientnetv2_s', pretrained=True,num_classes=config.n_classes)
# model = FineTunedEffNetV2(config.n_classes)
# model = ConvNet(config.n_classes)
# model = FineTunedResNetV1(config.n_classes)
model = FineTunedDenseNet(config.n_classes)
print(model)
...
for epoch in range(0,config.epochs):
    ...
    ## Saving the model
    filename = "models/Knife-DenseNet121_adamw-E" + str(epoch + 1)+  ".pt"
